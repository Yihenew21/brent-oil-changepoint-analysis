...
def build_changepoint_model(data: np.ndarray, n_days: int) -> pm.Model:
    ...
    with pm.Model() as model:
        # Define prior for switch point (tau) as continuous uniform, rounded to integer
        tau_cont = pm.Uniform("tau_cont", lower=0, upper=n_days - 1)
...
def run_mcmc(
    model: pm.Model, draws: int = 500, tune: int = 500
    ...
    """
    with model:
        # Use Metropolis for tau_cont, NUTS for continuous parameters
        step1 = pm.Metropolis(vars=[model.tau_cont])
        step2 = pm.NUTS(vars=[model.mu_1, model.mu_2, model.sigma])
        trace = pm.sample(
            draws=draws, tune=tune, step=[step1, step2], return_inferencedata=False
        )
...
def interpret_changepoint(
    trace: pm.backends.base.MultiTrace, dates: pd.Series, events_df: pd.DataFrame
) -> dict:
    """
